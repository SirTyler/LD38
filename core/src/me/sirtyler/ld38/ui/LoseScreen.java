...
public class LoseScreen {
	...
	public LoseScreen() {
		imgA = new Texture("lose_A.png");
		imgB = new Texture("lose_B.png");
		
		center = new Vector2(Gdx.graphics.getWidth()/2, Gdx.graphics.getHeight()/2);
		
		buttons[0] = new Button(Gdx.graphics.getWidth()/2-160,Gdx.graphics.getHeight()/2-144, "play");
		buttons[1] = new Button(Gdx.graphics.getWidth()/2+160,Gdx.graphics.getHeight()/2-144, "quit");
	...
	public void render(OrthographicCamera camera, SpriteBatch batch) {
		cam = camera;
		LD38Game.world.resetCamera(camera);
		camera.update();
		...
		font.draw(batch, "Game Over", (camera.viewportWidth/2)+32, camera.viewportHeight-64);
